...
``` clojure
(def 상수-리스트-오브젝트 '(:위스키병 :양동이 :개구리 :사슬))
```
 ...
 이 명령어는 주어진 값으로 변수를 설정합니다: 변수 이름은 `상수-리스트-오브젝트`이며, 키워드로된 오브젝트들이 리스트안에 들어있습니다.
 편의상 변하지 않는 변수 이름 앞에 `상수-`를 붙였습니다.
...
이제 게임 세상을 표현하는 `상수-지도`라는 새로운 변수를 정의해 봅시다:
...
여기서, 사전을 만들어서 오브젝트랑 장소를 각각 연결하였습니다. 사전(dictionary)은 키와 값으로 되어있는 컬렉션(collection)입니다. 리스트와 비슷하게 중괄호(`{ }`)로 둘러싸여 있습니다.
